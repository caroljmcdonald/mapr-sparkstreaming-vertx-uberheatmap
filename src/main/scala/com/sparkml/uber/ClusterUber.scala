...
object ClusterUber {
  ...
  def main(args: Array[String]) {
   ...
   // case class Uber(dt: String, lat: Double, lon: Double, base: String) extends Serializable
    ...
    // Spark 2.0
   //  val ds: Dataset[Uber] = spark.read.option("inferSchema", "false").schema(schema).csv("/user/user01/data/churn-bigml-80.csv").as[Uber]
      
    val df = spark.read.option("inferSchema", "false").schema(schema).csv("/user/user01/data/uber.csv") 
